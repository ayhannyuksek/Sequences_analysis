working_dir = "/media/ayhan/Elements/Sequence_Analysis/de/htseq_counts"
setwd(working_dir)

#Read in gene mapping
mapping=read.table("/media/ayhan/Elements/Sequence_Analysis/de/htseq_counts/ENSG_ID2Name.txt", header=FALSE, stringsAsFactors=FALSE, row.names=1)

# Read in count matrix
rawdata=read.table("/media/ayhan/Elements/Sequence_Analysis/expression/htseq_counts/gene_read_counts_table_all_final.tsv", header=TRUE, stringsAsFactors=FALSE, row.names=1)
...
# make class labels
class <- factor( c( rep("NORMAL",12), rep("ASD",4) ))
