...
outfile="/media/ayhan/Elements/Sequence_Analysis/de/ballgown/ref_only/Tutorial_Part2_ballgown_output.pdf"

# Load phenotype data
pheno_data = read.csv("NORMAL_vs_ASD.csv")
...
ballgown::transcriptNames(bg)[181173]

# Display the gene name for a single row of data 
ballgown::geneNames(bg)[181173]

# Create a BoxPlot comparing the expression of a single gene for all replicates of both conditions
plot(fpkm[181173,] ~ pheno_data$type, border=c(2,3), main=paste(ballgown::geneNames(bg)[181173],' : ', ballgown::transcriptNames(bg)[181173]),pch=19, xlab="Type", ylab='log2(FPKM+1)')

# Add the FPKM values for each sample onto the plot 
points(fpkm[181173,] ~ jitter(as.numeric(pheno_data$type)), col=as.numeric(pheno_data$type)+1, pch=16)

# Create a plot of transcript structures observed in each replicate and color transcripts by expression level
plotTranscripts(ballgown::geneIDs(bg)[181173], bg, main=c('Gene in sample HBR_Rep1'), sample=c('HBR_Rep1'))
plotTranscripts(ballgown::geneIDs(bg)[181173], bg, main=c('Gene in sample HBR_Rep2'), sample=c('HBR_Rep2'))
plotTranscripts(ballgown::geneIDs(bg)[181173], bg, main=c('Gene in sample HBR_Rep3'), sample=c('HBR_Rep3'))
plotTranscripts(ballgown::geneIDs(bg)[181173], bg, main=c('Gene in sample UHR_Rep1'), sample=c('UHR_Rep1'))
plotTranscripts(ballgown::geneIDs(bg)[181173], bg, main=c('Gene in sample UHR_Rep2'), sample=c('UHR_Rep2'))
plotTranscripts(ballgown::geneIDs(bg)[181173], bg, main=c('Gene in sample UHR_Rep3'), sample=c('UHR_Rep3'))
