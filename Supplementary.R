...
working_dir = "/media/ayhan/Elements/Sequence_Analysis/de/ballgown/ref_only" 
setwd(working_dir)
...
gene_expression[1:3,c(1:3,13)]

#Do the same thing, but using the column names instead of numbers
gene_expression[1:3,c("FPKM.SRR14881654","FPKM.SRR14881655","FPKM.SRR14881656","FPKM.SRR14881666")]
...
i = row.names(gene_expression) == "ENSG00000175309"
gene_expression[i,]

#What if we want to view values for a list of genes of interest all at once? 
#genes_of_interest = c("TST", "MMP11", "LGALS2", "ISX")
genes_of_interest = c("ENSG00000175309","ENSG00000174600","ENSG00000167995","ENSG00000118971")
...
min(gene_expression[,"FPKM.SRR14881654"])
max(gene_expression[,"FPKM.SRR14881654"])
...
data_columns=c(1:16)
short_names=c("SRR14881654","SRR14881655","SRR14881656",
              "SRR14881657","SRR14881658","SRR14881659","SRR14881660",
              "SRR14881661","SRR14881662","SRR14881663","SRR14881664",
              "SRR14881665","SRR14881666","SRR14881667","SRR14881668","SRR14881669")
...
x = gene_expression[,"FPKM.SRR14881654"]
y = gene_expression[,"FPKM.SRR14881655"]
plot(x=log2(x+min_nonzero), y=log2(y+min_nonzero), pch=16, col="blue", cex=0.25, xlab="FPKM (NORMAL, SRR14881654)", ylab="FPKM (NORMAL, SRR14881655)", main="Comparison of expression values for a pair of replicates")#Add a straight line of slope 1, and intercept 0
abline(a=0,b=1)
...
smoothScatter(x=log2(x+min_nonzero), y=log2(y+min_nonzero), xlab="FPKM (NORMAL, SRR14881654)", ylab="FPKM (NORMAL,SRR14881655)", main="Comparison of expression values for a pair of replicates", colramp=colors, nbin=200)
...
plotCor("FPKM.SRR14881654", "FPKM.SRR14881666", "SRR54 vs SRR66", "tomato2")
plotCor("FPKM.SRR14881655", "FPKM.SRR14881667", "SRR55 vs SRR67", "royalblue2")
...
par(mfrow=c(1,2))
plotCor2("FPKM.SRR14881654", "FPKM.SRR14881666", "SRR54 vs SRR66", "tomato2")
plotCor2("FPKM.SRR14881655", "FPKM.SRR14881667", "SRR55 vs SRR67", "royalblue2")
...
hist(results_genes[sig,"de"], breaks=50, col="seagreen", xlab="log2(Fold change) NORMAL vs ASD", main="Distribution of differential expression values")
...
gene_expression[,"NORMAL"]=apply(gene_expression[,c(1:12)], 1, mean)
gene_expression[,"ASD"]=apply(gene_expression[,c(13:16)], 1, mean)


x=log2(gene_expression[,"NORMAL"]+min_nonzero)
y=log2(gene_expression[,"ASD"]+min_nonzero)
plot(x=x, y=y, pch=16, cex=0.25, xlab="NORMAL FPKM (log2)", ylab="ASD FPKM (log2)", main="NORMAL vs ASD FPKMs")
abline(a=0, b=1)
xsig=x[sig]
ysig=y[sig]
